...
export default{
    ...
    data(){
        return{
            domain:'https://www.rakheemansukhani.com/',
            ablogs:[],
    ...
    methods:{
        ...
        addBlogData(){
            ...
            axios.post('/api/admin/rblog/add',adata,uheaders)
                .then((resp)=>{
                    if(resp.data.status){
                        const rblog_id = resp.data.rblog.rblog_id;
                        this.$router.push({name:'rblogedit',params:{rblog_id:rblog_id}})
                    }
                })
                .catch((err)=>{
                    alert(err.message);
