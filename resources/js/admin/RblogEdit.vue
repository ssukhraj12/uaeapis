    ...
    <div class="blogs">
        <v-row>
            ...
            <v-col cols="12" md="6" class="text-end">
                <v-btn :to="{name:'rblogs'}" density="default" class="text-none me-1" prepend-icon="mdi-arrow-left"
...
export default{
    name:"RblogEdit",
    props:{
        rblog_id:[Number,String]
    },
    data(){
        return{
            domain:'https://www.rakheemansukhani.com/blog/',
            ablog:{},
            ablogs:[],
    ...
    methods:{
        getBlogtoEdit(){
            axios.get('/api/admin/rblog/update/'+this.rblog_id)
                .then((resp)=>{
                    this.ablog = resp.data.rblog;
                    this.blog_title = resp.data.rblog.blog_title;
                    this.blog_status = resp.data.rblog.blog_status;
                    this.tags = resp.data.rblog.tags;
                    this.existingImageName  = resp.data.rblog.blog_image;
                    if (this.existingImageName) {
                        this.blog_image_url = `/${this.existingImageName}`;
                        this.imagePreview = this.blog_image_url;
                    }
                    this.meta_title = resp.data.rblog.meta_title;
                    this.meta_desc = resp.data.rblog.meta_desc;
                    this.quillContent = resp.data.rblog.blog_description;
        ...
        updateBlogData(){
            ...
            axios.post('/api/admin/rblog/update/'+this.rblog_id,ndata,uheaders)
                .then((resp)=>{
                    this.getBlogtoEdit();
                })
                .catch((err)=>{
